...
class TrajectoryGenerator:
    ...
    def __init__(
        ...
        if self._mult_add_pose_msg.poses[-1].fine != 0:
            ...
            self._logger.warning(
                " proc_planner: last waypont must have fine param set to 0"
            )
        ...
        self._n = (
            len(self._mult_add_pose_msg.poses) + supp_point + self._curr_pos_offset + 1
        )
        ...
        if self._status == self.TrajectoryStatus.RECEIVED_VALID_WAYPTS and bool(
            self._compute_time_arrival()
        ):
            self._nb_points = int(
                np.round(self._time_list[-1][0] / self._ros_param["ts"])
            )
        else:
            self._nb_points = 1
    ...
    def _get_initial_waypoint(self, curr_pos_msg: geoPose) -> bool:
        # Fill the lists
        self._point_list[0, :] = np.array(
            [curr_pos_msg.position.x, curr_pos_msg.position.y, curr_pos_msg.position.z]
        )
    ...
    def _interp_strategy(
        ...
        traj_list = None
        if (
            self._mult_add_pose_msg.interpolation_method
            == PoseArray.INTERPOLATION_HERMITE
        ):
            if not verif:
            ...
            self._mult_add_pose_msg.interpolation_method
            == PoseArray.INTERPOLATION_V5CUBIC
        ):
            if not verif:
            ...
            self._mult_add_pose_msg.interpolation_method
            == PoseArray.INTERPOLATION_SPLINE
        ):
            if not verif:
                sample_indices = (sample - time_list[0]) / (
                    time_list[-1] - time_list[0]
                )
                traj_list = map_coordinates(
                    point_list, sample_indices, order=3, mode="nearest"
                )
            else:
                traj_list = 1
        else:
    ...
    def _process_waypoints(self) -> None:
        for i, pose in enumerate(self._mult_add_pose_msg.poses):
            q = Rotation.from_euler(
                "zyx",
                np.deg2rad(
                    [pose.orientation.z, pose.orientation.y, pose.orientation.x]
                ),
            ...
            if frame == soniaPose.FRAME_ABS_POS_ABS_ANG:
                ...
                self._quat_list[i + self._curr_pos_offset, :] = (
                    Rotation.from_quat(
                        self._quat_list[i + self._curr_pos_offset - 1, :]
                    )
            ...
            if i > 0 and self._mult_add_pose_msg.poses[i - 1].fine != 0:
                ...
                # Declare waypoints
                self._point_list[i + self._curr_pos_offset + 1, :] = self._point_list[
                    i + self._curr_pos_offset, :
                ]
                self._point_list[i + self._curr_pos_offset, :] = p12
                self._point_list[i + self._curr_pos_offset - 1, :] = p01

                self._quat_list[i + self._curr_pos_offset + 1, :] = self._quat_list[
                    i + self._curr_pos_offset, :
                ]
                self._quat_list[i + self._curr_pos_offset, :] = self._quat_list[
                    i + self._curr_pos_offset - 1, :
                ]
                self._quat_list[i + self._curr_pos_offset - 1, :] = self._quat_list[
                    i + self._curr_pos_offset - 2, :
                ]

                self._speed_list[0, i + self._curr_pos_offset + 1] = self._speed_list[
                    0, i + self._curr_pos_offset
                ]
                self._speed_list[0, i + self._curr_pos_offset] = self._speed_list[
                    0, i + self._curr_pos_offset - 1
                ]

                self._course_list[0, i + self._curr_pos_offset + 1] = self._course_list[
                    0, i + self._curr_pos_offset
                ]
                self._course_list[0, i + self._curr_pos_offset] = self._course_list[
                    0, i + self._curr_pos_offset - 1
                ]
                self._course_list[0, i + self._curr_pos_offset - 1] = self._course_list[
    ...
    def _compute_time_arrival(self) -> bool:
        for i in range(1, self._n):
            ...
            q_rel = (
                Rotation.from_quat(np.conj(self._quat_list[i - 1, :])).inv()
                * Rotation.from_quat(self._quat_list[i, :])
            ).as_quat()
    ...
    def _interpolate_waypoints(self):
        ...
        self._traj_position[:, 0] = self._interp_strategy(
            self._time_list[:, 0], self._point_list[:, 0], t, False
        ).T
        self._traj_position[:, 1] = self._interp_strategy(
            self._time_list[:, 0], self._point_list[:, 1], t, False
        ).T
        self._traj_position[:, 2] = (
            PchipInterpolator(self._time_list[:, 0], self._point_list[:, 2])(t)
        ).T

        self._traj_body_velocity[:, 0] = np.append(
            [0], np.diff(self._traj_position[:, 0])
        )
        self._traj_body_velocity[:, 1] = np.append(
            [0], np.diff(self._traj_position[:, 1])
        )
        self._traj_body_velocity[:, 2] = np.append(
            [0], np.diff(self._traj_position[:, 2])
        )

        self._traj_linear_acceleration[:, 0] = np.append(
            [0], np.diff(self._traj_body_velocity[:, 0])
        )
        self._traj_linear_acceleration[:, 1] = np.append(
            [0], np.diff(self._traj_body_velocity[:, 1])
        )
        self._traj_linear_acceleration[:, 2] = np.append(
            [0], np.diff(self._traj_body_velocity[:, 2])
        )

        self._traj_quat[:, 0] = PchipInterpolator(
            self._time_list[:, 0], self._quat_list[:, 0]
        )(t).T
        self._traj_quat[:, 1] = PchipInterpolator(
            self._time_list[:, 0], self._quat_list[:, 1]
        )(t).T
        self._traj_quat[:, 2] = PchipInterpolator(
            self._time_list[:, 0], self._quat_list[:, 2]
        )(t).T
        self._traj_quat[:, 3] = PchipInterpolator(
            self._time_list[:, 0], self._quat_list[:, 3]
        )(t).T
        ...
        for i in range(self._nb_points):
            self._traj_quat[i, :] = self._traj_quat[i, :] / np.linalg.norm(
                self._traj_quat[i, :]
            )
            ...
            self._traj_body_velocity[i, :] = Rotation.from_quat(
                self._traj_quat[i, :]
            ).apply(self._traj_body_velocity[i, :])

            self._traj_angular_rates[i, :] = self._quat_to_angular_rates(
                self._traj_quat[i, :], qdot[i, :]
            )

            self._traj_angular_rates[i, :] = -self._traj_angular_rates[i, :]

            self._traj_linear_acceleration[i, :] = Rotation.from_quat(
                self._traj_quat[i, :]
            ).apply(self._traj_linear_acceleration[i, :])

        self._traj_anglular_acceleration[:, 0] = np.append(
            [0], np.diff(self._traj_angular_rates[:, 0])
        )
        self._traj_anglular_acceleration[:, 1] = np.append(
            [0], np.diff(self._traj_angular_rates[:, 1])
        )
        self._traj_anglular_acceleration[:, 2] = np.append(
            [0], np.diff(self._traj_angular_rates[:, 2])
        )
    ...
    def _quat_to_angular_rates(q, qDot):
        ...
        E2 = liniairzeE2(q)
        invE2 = np.linalg.pinv(E2)  # Pseudo-inverse of E2

        # Calculate the angular rates
        angularRates = -(invE2 @ qDot)  # Matrix multiplication
